...
void init(JNIEnv* env) {
    ...
    getServiceMethod = env->GetStaticMethodID(serviceManagerClass, "getService",
    ...
    transactMethod =
        env->GetMethodID(iBinderClass, "transact", "(ILandroid/os/Parcel;Landroid/os/Parcel;I)Z");

    // Parcel
    auto parcelClass_ = env->FindClass("android/os/Parcel");
    if (parcelClass_)
        parcelClass = (jclass)env->NewGlobalRef(parcelClass_);
    ...
    writeInterfaceTokenMethod =
        env->GetMethodID(parcelClass, "writeInterfaceToken", "(Ljava/lang/String;)V");
    ...
    readStrongBinderMethod =
        env->GetMethodID(parcelClass, "readStrongBinder", "()Landroid/os/IBinder;");
    ...
    if (deadObjectExceptionClass_)
        deadObjectExceptionClass = (jclass)env->NewGlobalRef(deadObjectExceptionClass_);
}
...
static jobject requireBinder(JNIEnv* env, bool force = false) {
    ...
    */
    ...
    bridgeService =
        env->CallStaticObjectMethod(serviceManagerClass, getServiceMethod, bridgeServiceName);
    if (!bridgeService) {
    ...
    res = env->CallBooleanMethod(bridgeService, transactMethod, BRIDGE_TRANSACTION_CODE, data,
                                 reply, 0);
